...
/**************************************************************************
* added prototypes for new refactored functions
**************************************************************************/
int refac_baron(struct gameState *state, int choice1, int currentPlayer);
int refac_minion(struct gameState *state, int handPos, int currentPlayer, int choice1, int choice2);
int refac_ambassador(struct gameState *state, int choice1, int choice2, int handPos, int currentPlayer);
int refac_tribute(struct gameState *state, int currentPlayer);
int refac_mine(struct gameState *state, int currentPlayer, int choice1, int choice2, int handPos);
...
//Refactored baron function and added two bugs
int refac_baron(struct gameState *state, int choice1, int currentPlayer){
  ...
  if (choice1 > 0) { //Boolean true or going to discard an estate
      ...
      while(card_not_discarded) {
          if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
              //first bug in baron, changed to += 3 from += 4
          ...
          //second bug in baron, changed p > state to p < state
...
int refac_minion(struct gameState *state, int handPos, int currentPlayer, int choice1, int choice2){
  ...
  else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
  {
      ...
      //first bug added: changed (state) > 0 to (state) < 0
      //discard hand
      ...
      for (i = 0; i < state->numPlayers; i++)
      {
          if (i != currentPlayer)
          {
              if ( state->handCount[i] > 4 )
              {
                  ...
                  //second bug added: changed j < 4 to j <= 4
                  //draw 4
                  for (j = 0; j <= 4; j++)
                  {
                      drawCard(i, state);
...
int refac_ambassador(struct gameState *state, int choice1, int choice2, int handPos, int currentPlayer){
  ...
  {
    //first bug added: change from return -1 to return 0
      return 0;
  }
  ...
  //second bug added: changed i++ to i--
...
int refac_tribute(struct gameState *state, int currentPlayer){
  ...
  //first bug added:  changed <= 1 to >= 1
  ...
  for (i = 0; i <= 2; i++) {
      if (tributeRevealedCards[i] == copper || tributeRevealedCards[i] == silver || tributeRevealedCards[i] == gold) { //Treasure cards
          //second bug added: changed coins += 2 to -= 2
          state->coins -= 2;
      }
...
int refac_mine(struct gameState *state, int currentPlayer, int choice1, int choice2, int handPos){
  ...
  //first bug added: removed int from int j = state
